...
void pieces_move(int x,int y,int moving){
    //If it is a covered piece, flip it. (I.e. make it positive.)
    if(board[x][y]<0){
        board[x][y]=-board[x][y];
        return;
    }
    //If it is an uncovered piece, move it to the direction where the user wants to
    if(board[x][y]>0){
        int nx=x+movingX[moving];
        int ny=y+movingY[moving];
